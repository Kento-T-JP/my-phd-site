...
export default function Formation({
  ...
  const [flippedPlayerId, setFlippedPlayerId] = useState<number | null>(null);
  ...
  const startFlipTimer = (id: number) => {
    if (flipTimer.current) clearTimeout(flipTimer.current);
    flipTimer.current = setTimeout(() => {
      setFlippedPlayerId(id);
    }, 500);
  };
  ...
  useEffect(() => {
    const up = () => setFlippedPlayerId(null);
  ...
  const renderBenchCard = (p: Player) => (
    <div
      key={p.id}
      className={`group relative w-32 max-w-32 max-h-32 cursor-pointer transition-transform duration-200 hover:scale-105 rounded ${
        selectedId === p.id ? "ring-2 ring-cyan-300" : ""
      }`}
      tabIndex={0}
      role="button"
      onPointerDown={() => startFlipTimer(p.id)}
      onPointerUp={() => handlePointerUp(p.id)}
      onPointerLeave={cancelFlipTimer}
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();
          setFlippedPlayerId((prev) => (prev === p.id ? null : p.id));
        }
      }}
      onClick={() => {
        ...
        handleClick(p.id, true);
      }}
    >
      <div
        className={`w-full h-full [transition:transform_0.6s] [transform-style:preserve-3d] ${
          flippedPlayerId === p.id
            ? "[transform:rotateY(180deg)] shadow-lg shadow-cyan-500/50"
            : ""
        }`}
      >
        <div className="absolute inset-0 p-2 border rounded backdrop-blur-sm border-cyan-400/10 hover:border-cyan-400/20 bg-slate-800/50 [backface-visibility:hidden]">
        ...
        <div className="absolute inset-0 p-2 border rounded backdrop-blur-sm border-cyan-400/10 bg-slate-800/50 flex items-center justify-center [transform:rotateY(180deg)] [backface-visibility:hidden]">
          <WikiLink name={p.name} wikiUrl={p.wikiUrl} variant="cta" />
        </div>
      </div>
    </div>
  );
        ...
        {sortedKeys.map((posKey) => {
          ...
          return group.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`absolute group w-32 max-w-32 max-h-32 cursor-pointer transition-transform duration-200 hover:scale-105 rounded ${
                  selectedId === p.id ? "ring-2 ring-cyan-300" : ""
                }`}
                style={{
                  top: `${pos.top}%`,
              ...
              >
                <div
                  className={`w-full h-full p-2 text-center backdrop-blur-sm border border-cyan-400/10 hover:border-cyan-400/20 bg-slate-800/50 rounded [transition:transform_0.6s] [transform-style:preserve-3d] ${
                    flippedPlayerId === p.id
                      ? "[transform:rotateY(180deg)] shadow-lg shadow-cyan-500/50"
                      : ""
                  }`}
