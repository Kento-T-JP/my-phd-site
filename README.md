...
補足:
- まれに `prisma migrate deploy` の advisory lock 競合（`P1002`）で失敗することがあります
  - この場合は `redeploy` で解消するケースがあります
- 将来的には `prisma migrate deploy` を Build Command から分離し、CI/CD の単独ジョブで直列実行する運用も検討対象です
