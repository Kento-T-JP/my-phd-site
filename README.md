...
- フォーメーションの共同編集（専用画面でリアルタイム同期、必要時のみ通常保存）
- 選手の登録/編集/削除、JFA URL / Excel 取り込み
- 大会・メンバーリスト管理とフィルター連携
- お気に入り選手管理
- 管理者向けの運用画面（ユーザー、問い合わせ、統計、フォーメーション、メンバーリスト）
...
- ロングテキスト（大会名/メンバーリスト名）は読める表示を優先
- 単独編集画面と共同編集画面を分離し、役割を明確化
  - 通常の `/formations` は保存・更新中心の単独編集
  - `/formations/collaborative/[id]` は共同編集専用画面としてリアルタイム同期・編集中表示を提供
  - 共同編集画面の変更は恒久保存せず、必要な場合のみ通常フォーメーションとして保存
  - 共同編集は「移動イベント」と「確定スナップショット」を分離し、リアルタイム性と安定性を両立
...
- 共有取り込み時、既存大会・メンバーリスト（内部モデル: Roster）を upsert で再利用して重複を抑制
- フォーメーション `nodes` の読み順を固定（`id asc`）
- 日付処理をユーティリティ化し、表示・保存のブレを抑制
- フォーメーション単位で共同編集者を管理し、所有者/共同編集者の権限を分離
- 共同編集者も所有者スコープの選手・大会・メンバーリストを参照できるよう統一
- 共同編集イベントは用途別にチャンネル分離し、移動イベントのノイズで確定同期が詰まりにくい構成に調整
...
- 複数メンバーリスト紐付けを一括 INSERT 化し、書き込み効率を改善
- パフォーマンス計測ログは環境変数で明示的に有効化（通常OFF）
- Safari / Chrome の描画実装差を前提に、Safari向けの軽量モードを導入
  - Safari: 背景パーティクルは静止描画（点と線は表示、連続アニメーションなし）
  - Safari: `backdrop-filter` を抑制し、ガラス表現の負荷を軽減
  - Chrome系: 従来どおり滑らかなアニメーションを維持
- 背景Canvasをリサイズ耐性化
  - リサイズ中は描画を抑制/停止し、確定後に再開
  - ビューポート追従で粒子を再配置し、サイズ変更後の「粒子消失」や負荷残留を抑制
  - 接続線計算を近傍探索ベースへ変更し、計算量を削減

#### 主要ボトルネックと解消（実運用での学び）

- ボトルネック1: DBリージョン距離によるレイテンシ
  - 当初、Vercel（北米東海岸）に対して Neon DB がシンガポールリージョンにあり、往復通信遅延が発生
  - DB をアプリ実行リージョンに近いリージョンへ移行し、体感速度を改善
- ボトルネック2: 背景描画のブラウザ差（Safari）
  - Safari でのみ重さが目立ち、背景エフェクトと描画負荷が原因
  - Safari向けに背景処理を軽量化（静止描画・blur負荷抑制）し、Chromeに近い体感まで改善
- 結果
  - 選手を100名以上登録した状態でも、日常操作でほぼノンストレスなレスポンスを維持
...
- Quick Actionにキーボード操作とフォーカス管理を実装
  - `Esc` で閉じる
...
- 共同編集はフォーメーション単位の許可ユーザー制御
  - 所有者のみ共同編集者を設定可能
  - 共同編集者は許可されたフォーメーションのみ取得・更新可能
...
- 共有リンク作成・閲覧・取り込み
- 共同編集専用ページ
  - 特定フォーメーションごとに共同編集者を設定
  - 変更をリアルタイム同期
  - 現在編集中ユーザーを表示
  - 編集内容はその場限りで共有し、必要時のみ通常フォーメーションとして保存
  - 選手ドラッグは軽量イベントで即時共有し、交換・リセット・テンプレ切替は確定状態を即時同期
  - 競合時は last-write-wins で整合を維持

### 選手・大会・メンバーリスト

- 選手の新規登録/編集/削除（複数削除対応）
- JFA URL 取り込み、Excel (`.xlsx`) 取り込み
- 大会・メンバーリストのユーザースコープ管理
- お気に入りフィルター
...
共同編集のリアルタイム同期には Ably を利用します。`.env.local` と本番環境に `ABLY_API_KEY` を設定してください。

Ably のセットアップメモ:

- ログイン方法は GitHub アカウント
- 登録メールアドレスは `kentoh120901@gmail.com`
- App 作成時は `Pub/Sub` を選択
- API key は `Root` を利用し、`ABLY_API_KEY` に設定
- 共同編集は `formation-state:*` と `formation-move:*` の2系統で配信し、移動イベントと確定同期を分離

テストをコンテナ内で実行する場合:

```bash
docker compose run --rm app sh -lc "npx prisma migrate deploy && npm test"
```
...
- 大会 / メンバーリスト（内部モデル: Roster） / フォーメーション: 削除時は物理削除
