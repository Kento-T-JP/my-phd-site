...
- ポジション削除の連動反映（選手/ロースター紐付けにも適用）
- 選手の新規登録・編集・削除（複数削除含む）
- 新規登録の安定化（CSRF再取得 + 403時リトライ）
- 新規登録時の規約同意フロー（利用規約/プライバシーポリシー同意必須）
- 規約ページ遷移後も新規登録フォーム入力を保持
...
- 新規選手登録の送信時に CSRF を毎回再取得し、403時に再送するフェイルセーフを追加
- ポジション削除時に `Player.position` / `RosterPlayer.position` も同期更新
...
- ポジション管理（`/positions`）をユーザー単位で実装
  - デフォルト表示・削除不可
  - カスタム追加/削除可
  - カスタム削除時は選手・ロースター紐付けのポジション配列も連動更新
- ポジション表記を `LB/RB` から `LSB/RSB` に整理（旧入力は互換変換）
- 新規登録・編集画面のポジションUIをジャンル別表示に統一
  - `DF`: `DF, CB, LSB, RSB`
  - `MF`: `DMF, CMF, LM, RM, OMF`（`MF`は補助扱い）
  - `FW`: `LW, RW, CF`（`FW`は補助扱い）
  - `Other`: `MF/FW`
- 大会管理をロースター追加導線に集約（大会名必須、ロースター名任意）
- SEO基盤（metadata/OG/Twitter/robots/sitemap）を整備
- 管理者ユーザー一覧で規約同意情報を可視化
  - `Terms Accepted At`
  - `Privacy Accepted At`
  - `Legal Version`
...
- `/terms`（利用規約）と `/privacy`（プライバシーポリシー）を追加
- 新規登録APIで同意フラグと適用版（`LEGAL_VERSION`）を検証
- `PendingRegistration` に同意時刻と版を保持し、メール認証完了後に `User` へ引き継ぎ
- 新規登録確認メール / 問い合わせ受付メール / 管理者通知メールを日英二言語に統一
